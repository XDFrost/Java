...
// Java assigns priority to threads from 1 to 10. 1 being the lowest and 10 being the highest.
// if multiple threads have the same priority, round robin scheduling is used.


// context switching is the process of storing and restoring the state of a thread so that execution can be resumed from the same point at a later time.
// Rules:
...
//Every object in Java has a monitor associated with it ; THERE IS NO MONITOR CLASS IN JAVA
...
// Once a java program starts, one thread begins running immediately. This is usually called the main thread of the program.
// The main thread is important because it is the thread from which other "child" threads will be spawned.
...
class imple implements Runnable {
    Thread t;
    imple() {
        t = new Thread(this, "Demo Thread");            // this is the reference to the current object; this referes to the run method of current class ; if this is not used, run method won't be overridden and the code will be executed in the main thread
...
// In above, main is the name of the thread, 5 is the priority of the thread, and main is the name of the thread again

// Currenthread: Thread[Thread-0,5,main] 1
// In above, Thread-0 is the name of the thread, 5 is the priority of the thread, and main is the name of the thread again
